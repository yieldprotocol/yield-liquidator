...
mod uniswapv2pair_mod {
    ...
    #[derive(Clone)]
    pub struct UniswapV2Pair<P, S>(Contract<P, S>);
    impl<P, S> std::ops::Deref for UniswapV2Pair<P, S> {
        type Target = Contract<P, S>;
        fn deref(&self) -> &Self::Target {
            &self.0
        }
    }
    impl<P: JsonRpcClient, S: Signer> std::fmt::Debug for UniswapV2Pair<P, S> {
    ...
    impl<'a, P: JsonRpcClient, S: Signer> UniswapV2Pair<P, S> {
        #[doc = r" Creates a new contract instance with the specified `ethers`"]
        #[doc = r" client at the given `Address`. The contract derefs to a `ethers::Contract`"]
        #[doc = r" object"]
        pub fn new<T: Into<Address>, C: Into<Arc<Client<P, S>>>>(address: T, client: C) -> Self {
            let contract = Contract::new(address.into(), UNISWAPV2PAIR_ABI.clone(), client.into());
            Self(contract)
        }
        ...
        pub fn approve(&self, spender: Address, value: U256) -> ContractCall<P, S, bool> {
            self.0
                .method_hash([9, 94, 167, 179], (spender, value))
                .expect("method not found (this should never happen)")
        }
        #[doc = "Calls the contract's `getReserves` (0x0902f1ac) function"]
        pub fn get_reserves(&self) -> ContractCall<P, S, (u128, u128, u32)> {
            self.0
        ...
        pub fn k_last(&self) -> ContractCall<P, S, U256> {
            self.0
        ...
        pub fn price_0_cumulative_last(&self) -> ContractCall<P, S, U256> {
            self.0
                .method_hash([89, 9, 192, 213], ())
                .expect("method not found (this should never happen)")
        }
        #[doc = "Calls the contract's `transfer` (0xa9059cbb) function"]
        pub fn transfer(&self, to: Address, value: U256) -> ContractCall<P, S, bool> {
            self.0
        ...
        pub fn swap_filter(&self) -> Event<P, SwapFilter> {
            self.0
                .event("Swap")
                .expect("event not found (this should never happen)")
        }
